...
登记 source-backed `Drop` 并生成递归 LLVM glue。scope exit 尚未物化 flag 或调用这些 glue。
...
这些 flag 和 obligation 仍是编译器内的 lowering 计划，不是已执行的析构。v0.15 已生成并通过原生
链接验证递归 LLVM glue，但 scope exit 尚未插入调用；LLVM flag storage/branch/call，以及 conditional
overwrite 的旧值析构将在下一步接入。
...
3. 从版本匹配的普通 core 源登记 `Drop`，生成递归 drop glue 并把清理 edge 降到 LLVM（v0.15 已完成
   lang item、实现约束与 glue，scope-exit lowering 待完成）；
4. 固定 raw pointer 语义和可替换 allocator ABI；
5. 在上述基础上加入拥有堆资源的 `alloc`；
6. 最后通过 C ABI 与最小运行时承载依赖宿主系统的 `std`。

当前 v0.14 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest、move-state fixed
point 与 temporary storage liveness，并完成第二步的 `needs_drop`、drop obligations 与 flag action
计划；v0.15 还完成第三步的 source-backed `Drop` 与递归 glue。Function capture environment、
match/pattern transfer、borrowed mutation、conditional maybe-overwrite cleanup 和 scope-exit LLVM
调用仍 pending，因此不能把 `CleanupPlan`、`StorageDead`、drop flag action 或任何 pending edge
当作已执行的析构。全局编译期常量仍按使用点重复物化且不参加
cleanup；资源/`Drop` 语义也必须在第三步开放 `Drop` 前明确。

## 20. 建议的分阶段范围

下列 M0–M4 表示能力之间的依赖层级，不与 `v0.x` 发布号或实际落地顺序一一对应。实现可以先完成
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.15 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
