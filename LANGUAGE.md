...
实现进度：v0.54 已解析 `borrow T` / `mut borrow T`，并允许它们标注由对应 borrow expression
初始化的局部 `let` 别名；泛型替换和模块类型遍历会保留 pointee。v0.55 又实现 `'a: region`、
`borrow('a)` / `mut borrow('a)` 的解析、全 item 作用域检查与单态化前擦除。v0.56 开放自由函数和
关联函数的显式 region 返回借用：返回来源必须是同 region 的借用参数，调用者将来源 loan 提升到
引用局部的词法作用域，并通过 LLVM 指针 ABI 支持间接读取、字段投影和可变赋值。当前仍拒绝借用值
参数、借用字段和局部或临时值逃逸。v0.57 将返回借用扩展到固有及 trait 方法，并在实例调用、类型限定
调用和泛型方法中提升 receiver loan。v0.58 实现唯一借用参数的省略 region 推断，并可依据外层显式
返回 region 对转发调用结果进行上下文化；零个或多个候选仍要求显式 region。v0.59 开放借用值参数，
区分引用 storage 读取与 pointee 自动解引用：共享引用可复制，可变引用会移动，二者均可参与泛型推断
和返回转发；引用值穿过产值块时会同步提升来源 loan。借用字段、引用 partial capture 与完整 NLL 尚未
实现。
...
传递模式、泛型约束和返回类型。v0.41 起实现可以省略带函数体的成员；默认体在 trait 定义处以
抽象 `Self: Trait` proof 检查，并在每个 concrete 实现上替换 `Self`、trait 参数和关联类型后静态
分派。实现中显式提供同名方法会覆盖默认体。
...
instance 不得同时获得二者。v0.40 又在 blanket 声明处抽象检查完整方法签名和方法体；检查环境只
假设声明的 where proofs，且不要求程序实际构造任何 concrete instance。
...
`unsafe do` 中调用。v0.61 另提供发散的 `raw_trap()`，供 alloc 等安全封装在动态前置条件失败时立即
终止；它同样只能出现在 `unsafe do` 中，返回 `never`，并降低为 `llvm.trap` 加 `unreachable`。null、
一般字节指针算术、`unsafe let`、属性和完整 C ABI 尚未实现。

v0.65 增加 `raw_borrow(pointer, borrow anchor)` 与
`raw_mut_borrow(pointer, mut borrow anchor)`。它们把 raw pointer 原样降低为共享或可变引用，并把
词法 loan 与返回 region 绑定到显式 anchor；编译器据此执行别名和逃逸检查。unsafe 调用者必须证明
pointer 指向 anchor 所拥有或保证存活的已初始化 `T`，且可变形式具有唯一访问权。
...
```sali
let mut values: Vec(i32) = Vec(i32).with_capacity(4)
values.push(20)
values.push(22)
let answer = values.read(0) + values.read(1)
```

首版 `Vec(T)` 仅为 `T: Copy` 提供构造与元素访问，API 包括 `new`、`with_capacity`、`len`、
`capacity`、`push`、`read` 和 `write`。pointer、length 与 storage capacity 都是 alloc 私有字段。
增长时先检查容量翻倍和 `capacity * size_of(T)`，再分配、逐元素复制、释放旧 storage；越界或布局
溢出会 trap。`Vec(())` 的地址不递增，但 length/capacity 与普通元素一致。最终 owner 通过普通
source-backed `Drop` 释放 allocation；资源元素将在定义逐元素 move/drop 与借用 slice 后开放。

v0.62 将 owning API 推广到资源元素。`new`、`with_capacity`、`push`、`replace` 和 `pop` 不要求
`Copy`；`push`/`replace` 的未标注 `T` 参数按普通规则对 Copy 值复制、对其他值移动。扩容使用
`raw_take`/`raw_init` 逐元素迁移，旧 allocation 只释放 storage。`replace` 返回被替换的 owner，
...
v0.65 增加 `at(index): borrow T` 与 `at_mut(index): mut borrow T`。方法只有一个借用来源，因此返回
...
v0.16–v0.26 明确覆盖的结构化 edge 已执行析构，不能把其余 `StorageDead` 或 drop flag action 当作
