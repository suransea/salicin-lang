...
`Fn` 是 `FnMut` 的子能力，`FnMut` 是 `FnOnce` 的子能力。命名函数和不捕获闭包通常实现 `Fn`
与 `Copy`。捕获方式本身不单独决定调用能力：一个 `move` 闭包若只读取其拥有字段，仍可实现 `Fn`；
...
编译器拒绝显式或推断签名泄漏私有实现类型。后续先在 `core` 完成运算、所有权、析构、指针和 slice
基础；有了 `Drop`、raw pointer 与分配器 ABI 后再引入 `alloc`，最后通过 C ABI 与最小运行时承载
依赖宿主系统的 `std`。

## 20. 建议的分阶段范围

下列 M0–M4 表示能力之间的依赖层级，不与 `v0.x` 发布号或实际落地顺序一一对应。实现可以先完成
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.6 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
