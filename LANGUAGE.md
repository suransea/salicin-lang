...
v0.9 实现中的顶层值仍是编译期常量：每次读取都会独立物化其值，不拥有一个参加函数
...
`_` 是局部类型推断占位符，不是一种类型，不能出现在导出的函数签名或结构体字段中。
...
普通函数调用默认按位置传参，不使用参数名作为外部标签。结构体构造器支持字段标签。
未来如果需要稳定的命名实参，建议单独引入外部标签语法，而不是让参数重命名破坏 API。
...
let names = map(_)(_)(users) { (user: User) -> user.name }
```
...
移动状态，并在 `Drop` 落地后据此生成 drop flags，不能因只在某个分支移动而重复析构。v0.9 已实现
编译期 move-path alternatives，但尚未生成 runtime drop flags 或析构。
...
`Copy`。v0.9 尚不支持 blanket/generic `Copy` impl 或 `where` 证明，函数类型和闭包类型自身也不是
...
同一版本会从实际 HIR 为每个函数构建并验证类型无关的 `CleanupPlan`。它包含词法、循环和 match-arm
scope，owned/borrowed local 与 move path，storage/init/move/overwrite 事件，以及真实控制流 edge。
该 plan 目前只验证结构，不能用于发出析构。`PendingCapability` 明确标记尚未物化的资源结果、move-path
state dataflow、temporary storage liveness、loop `break` value transfer、borrowed-place mutation、
`MaybeOverwrite`、match dispatch/pattern binding transfer，以及 partial application/closure capture。
`needs_drop`、runtime drop flags、source-backed `Drop`、drop glue 和 LLVM 析构均尚未实现。
...
实现 `Fn` 与 `Copy`；截至 v0.9，函数类型或闭包类型仍未实现 `Copy`。捕获方式本身不单独决定调用能力：
...
`Box(i32)` 是类型，随后一组括号才调用其构造器。`_` 由构造实参推断为 `i32`。
...
类型参数必须位于运行时参数组之前。`_` 可从后续参数和期望返回类型推断。无法唯一推断时报错，
不根据函数体外的任意隐式转换“猜测”类型。
...
`CleanupPlan`，但该 plan 还不会生成析构代码。

标准库权威路线保持 `core → alloc → std`，并按以下依赖顺序推进：

1. 在 `core` 阶段物化 cleanup storage，并完成 move-state dataflow 与 temporary/break/match/capture
   等当前 pending case；
2. 引入 `needs_drop` 判定和控制流敏感的 runtime drop flags；
3. 从版本匹配的普通 core 源登记 `Drop`，生成递归 drop glue 并把清理 edge 降到 LLVM；
4. 固定 raw pointer 语义和可替换 allocator ABI；
5. 在上述基础上加入拥有堆资源的 `alloc`；
6. 最后通过 C ABI 与最小运行时承载依赖宿主系统的 `std`。

当前 v0.9 只完成了第一步的 CFG/验证骨架，尚未完成其中的物化与 dataflow，更没有实现第二、三步。
因此不能把 `CleanupPlan`、`StorageDead` 记录或任何 pending edge 当作已执行的析构。全局编译期常量
仍按使用点重复物化且不参加 cleanup；资源/`Drop` 语义也必须在第三步开放 `Drop` 前明确。

## 20. 建议的分阶段范围

下列 M0–M4 表示能力之间的依赖层级，不与 `v0.x` 发布号或实际落地顺序一一对应。实现可以先完成
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.9 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
