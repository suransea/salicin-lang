...
当前 LLVM lowering 只物化完整 root flag。为避免把可观察析构做错，编译器暂时拒绝从需要 drop 的
聚合中移动字段、在部分移动后逐字段重建、把需要 drop 的 payload 移入 pattern binding、从临时
drop 值提取字段，以及让闭包环境拥有 drop 值。v0.14 的 projection obligation 仍保留，下一步会把
它们物化为细粒度 flags 后重新开放这些程序。conditional overwrite、borrowed-place mutation 与复杂
closure capture 的完整 cleanup 也仍列为 pending。
...
v0.15 加入 source-backed `Drop` 与递归 glue；v0.16 开始执行 root storage 的结构化 cleanup。
...
3. 从版本匹配的普通 core 源登记 `Drop`，生成递归 drop glue 并把清理 edge 降到 LLVM（v0.16 已完成
   root storage 的结构化 scope-exit lowering，projection/closure 等细节待补齐）；
4. 固定 raw pointer 语义和可替换 allocator ABI；
5. 在上述基础上加入拥有堆资源的 `alloc`；
6. 最后通过 C ABI 与最小运行时承载依赖宿主系统的 `std`。

当前 v0.16 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest、move-state fixed
...
return、break、match、overwrite 和 staging cleanup 降到 LLVM。Function capture environment、
projection partial drop、match payload transfer、borrowed mutation 和完整 conditional
maybe-overwrite cleanup 仍 pending；只有 v0.16 明确覆盖的结构化 edge 已执行析构，不能把其余
...
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.16 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
