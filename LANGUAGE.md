...
物化 struct projection flag tree；v0.18 接通直接 enum match payload transfer。
...
当前 v0.18 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest、move-state fixed
...
partial drop 与 conditional field rebuild，v0.18 补齐直接、无 guard 的 enum payload binding。
Function capture environment、嵌套/guarded enum pattern、borrowed mutation 和 closure cleanup 仍
pending；只有 v0.16–v0.18 明确覆盖的
