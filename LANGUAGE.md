...
cleanup IR，并移除全部 pending capability 标记。
...
当前 v0.24 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest、move-state fixed
...
操作，不再存在 pending capability 基础设施。escaping/first-class callable 仍缺少公开 ABI；只有
v0.16–v0.24 明确覆盖的结构化 edge 已执行析构，不能把其余 `StorageDead` 或 drop flag action 当作
