...
v0.14 又从这些状态生成 `needs_drop`、稳定 drop flag 及其 set/clear action 和作用域结束义务；LLVM
尚未物化这些 flag，也尚未调用析构。
...
这些 flag 和 obligation 仍是编译器内的 lowering 计划，不是已执行的析构。source-backed `Drop`、
递归 drop glue、LLVM flag storage/branch/call，以及 conditional overwrite 的旧值析构将在下一步接入。
...
计划；Function capture environment、match/pattern transfer、borrowed mutation 和 conditional
maybe-overwrite cleanup 仍 pending。第三步尚未实现，因此不能把 `CleanupPlan`、`StorageDead`、drop
flag action 或任何 pending edge 当作已执行的析构。全局编译期常量仍按使用点重复物化且不参加
cleanup；资源/`Drop` 语义也必须在第三步开放 `Drop` 前明确。

## 20. 建议的分阶段范围

下列 M0–M4 表示能力之间的依赖层级，不与 `v0.x` 发布号或实际落地顺序一一对应。实现可以先完成
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.14 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
