...
v0.10 实现中的顶层值仍是编译期常量：每次读取都会独立物化其值，不拥有一个参加函数
...
编译期 move-path alternatives；v0.10 补齐资源结果的稳定 storage 与转移，但尚未生成 runtime drop
flags 或析构。
...
`Copy`。截至 v0.10 尚不支持 blanket/generic `Copy` impl 或 `where` 证明，函数类型和闭包类型自身也不是
...
该 plan 仍只验证结构，不能用于发出析构。资源结果与 loop `break` value 已物化，旧的两项 pending
因此删除；`PendingCapability` 继续明确标记 move-path state dataflow、temporary storage liveness、
borrowed-place mutation、`MaybeOverwrite`、match dispatch/pattern binding transfer，以及 partial
application/closure capture。`needs_drop`、runtime drop flags、source-backed `Drop`、drop glue 和 LLVM
析构均尚未实现。
...
实现 `Fn` 与 `Copy`；截至 v0.10，函数类型或闭包类型仍未实现 `Copy`。捕获方式本身不单独决定调用能力：
...
`CleanupPlan`；v0.10 为资源结果补齐稳定 storage、聚合投影初始化与跨控制流转移，但该 plan 还不会
生成析构代码。

标准库权威路线保持 `core → alloc → std`，并按以下依赖顺序推进：

1. 在 `core` 阶段完成 move-path forest、cleanup state dataflow 与 temporary/match/capture 等当前
   pending case；
...
当前 v0.10 已完成第一步中的 CFG、结果 storage 与显式 transfer，但尚未完成 move-state dataflow 和
temporary/match/capture 细节，更没有实现第二、三步。因此不能把 `CleanupPlan`、`StorageDead` 记录
或任何 pending edge 当作已执行的析构。全局编译期常量仍按使用点重复物化且不参加 cleanup；资源/
`Drop` 语义也必须在第三步开放 `Drop` 前明确。

## 20. 建议的分阶段范围

下列 M0–M4 表示能力之间的依赖层级，不与 `v0.x` 发布号或实际落地顺序一一对应。实现可以先完成
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.10 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
