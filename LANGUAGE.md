...
v0.12 实现中的顶层值仍是编译期常量：每次读取都会独立物化其值，不拥有一个参加函数
...
完成静态 forest 和 `may_init`/`must_init` fixed point，v0.12 加入 local 的 `may_live`/`must_live`，但
尚未生成 runtime drop flags 或析构。
...
`Copy`。截至 v0.12 尚不支持 blanket/generic `Copy` impl 或 `where` 证明，函数类型和闭包类型自身也不是
...
partial application/local closure capture。该 plan 仍不能用于发出析构；`needs_drop`、runtime drop
flags、source-backed `Drop`、drop glue 和 LLVM 析构均尚未实现。
...
实现 `Fn` 与 `Copy`；截至 v0.12，函数类型或闭包类型仍未实现 `Copy`。捕获方式本身不单独决定调用能力：
...
循环逐轮 temporary scope，但该 plan 还不会生成析构代码。
...
当前 v0.12 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest、move-state fixed
point 与 temporary storage liveness；Function capture environment、match/pattern transfer、borrowed
mutation 和 conditional maybe-overwrite cleanup 仍 pending，第二、三步也尚未实现。因此不能把 `CleanupPlan`、
...
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.12 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
