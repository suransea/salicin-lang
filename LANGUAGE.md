...
移动整个字段值本身不受影响。需要 drop 的 enum payload pattern binding、临时 drop 值字段提取和让
闭包环境拥有 drop 值也仍明确拒绝。enum/downcast projection flags、match payload transfer、
borrowed-place mutation 与 closure capture cleanup 是下一阶段 pending。
...
物化 struct projection flag tree。
...
当前 v0.17 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest、move-state fixed
...
partial drop 与 conditional field rebuild。Function capture environment、enum/match payload
transfer、borrowed mutation 和 closure cleanup 仍 pending；只有 v0.16–v0.17 明确覆盖的
