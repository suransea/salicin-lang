...
v0.11 实现中的顶层值仍是编译期常量：每次读取都会独立物化其值，不拥有一个参加函数
...
完成静态 forest 和 `may_init`/`must_init` fixed point，但尚未生成 runtime drop flags 或析构。
...
`Copy`。截至 v0.11 尚不支持 blanket/generic `Copy` impl 或 `where` 证明，函数类型和闭包类型自身也不是
...
`MovePathStateDataflow` pending 已删除；`PendingCapability` 继续明确标记 temporary storage liveness、
conditional `MaybeOverwrite` cleanup、borrowed-place mutation、match dispatch/pattern binding transfer，
以及 partial application/local closure capture。该 plan 仍不能用于发出析构；`needs_drop`、runtime
drop flags、source-backed `Drop`、drop glue 和 LLVM 析构均尚未实现。
...
实现 `Fn` 与 `Copy`；截至 v0.11，函数类型或闭包类型仍未实现 `Copy`。捕获方式本身不单独决定调用能力：
...
完整静态 forest 并完成 control-flow move-state fixed point，但该 plan 还不会生成析构代码。
...
当前 v0.11 已完成第一步中的 CFG、结果 storage、显式 transfer、完整静态 forest 与 move-state
fixed point；Function capture environment、temporary liveness、match/pattern transfer、borrowed mutation
和 conditional maybe-overwrite cleanup 仍 pending，第二、三步也尚未实现。因此不能把 `CleanupPlan`、
`StorageDead` 记录或任何 pending edge 当作已执行的析构。全局编译期常量仍按使用点重复物化且不参加
cleanup；资源/`Drop` 语义也必须在第三步开放 `Drop` 前明确。

## 20. 建议的分阶段范围

下列 M0–M4 表示能力之间的依赖层级，不与 `v0.x` 发布号或实际落地顺序一一对应。实现可以先完成
后层的独立基础，再返回补齐前层依赖；当前 v0.5–v0.11 在 v0.4 的模块/包基础上开始收敛 M1–M2 的
