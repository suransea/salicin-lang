...
- `access` 和 `passing` 只在编译期 kind 位置具有上下文含义，不是全局保留字。
...
`unsafe { ... }` 处理并移除 `unsafe`，`do { ... }` 不处理 effect，只原样转发。后续的通用形式将以
`E: effect` 表示该返回参数组，例如 `T(E)` 或 `T(unsafe, E)`；不会引入方括号或 `_` 推断语法。
旧的 `T ! effect` 拼法在 1.0 前直接删除。
...
`type`、`region`、`access` 和 `passing` 描述编译期数据或调用约定；通用 effect 行使用独立的
`effect` kind 和约束规则。
