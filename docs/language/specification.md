...
这些内建能力也必须有 edition 固定的源码契约。`core.effects` 声明普通 effect 形态的 `Unsafe`、
带 `raise(move error: Error): Never` operation 的 `Throws(Error)`，以及带最小 `suspend(): ()`
operation 的 `Async`；`core.access` 声明 `Shared` 与 `Mutable`；`core.control` 声明 `do`、`try`、
