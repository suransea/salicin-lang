...
effect 是函数签名中独立的编译期参数组，写在所有运行时参数组之后、返回类型的 `:` 之前。没有
该组就是 pure；effect 的顺序不影响语义，同一项不能重复。它不接收运行时实参，也不增加一层
柯里化：

```sali
let read(pointer: Ptr(i32))(unsafe): i32 = *pointer
let optional()(try): i32 = 42
let fallible()(try(Error)): i32 = throw Error()
let combined(pointer: Ptr(i32))(try(Error), unsafe): i32 = *pointer
...
`(unsafe): T` 允许函数体执行原始指针等不安全操作，并要求完整调用发生在另一个含 `unsafe` 的函数或
...
签名 effect 组同时确定控制边界及其 ABI：`(try): T` 规范化为 `Option(T)`，
`(try(E)): T` 规范化为 `Result(T, E)`。所以裸 `try` 表示没有错误 payload 的可缺失结果，带错误类型
的 `try(E)` 表示可观察的错误结果。调用者取得普通 `Option` / `Result` 值；effect 不引入异常式的
隐藏运行时通道。显式写 `Option(T)`、`Result(T, E)` 仍用于普通值类型、自定义 Try API 以及需要
直接展示 carrier 的位置。
...
当前 `effect` kind 的实参是 `pure` 与 `unsafe`。它们不进入运行时，但参与单态化；`pure` 是省略且
没有其他约束时的默认值。effect 参数只能声明在函数或泛型 inherent member 上，只能用于返回
effect 组或转发给另一个 effect 编译期参数，不能作为运行时类型。`(unsafe, E): T` 表示固定要求
`unsafe` 再与 E 合并。选择 `unsafe` 的实例可在函数体执行 unsafe 操作；选择或默认 `pure` 的实例
仍会被相同的静态检查拒绝。不会引入方括号或 `_` 推断语法；旧的 `T ! effect` 拼法在 1.0 前直接
删除。

这里的 effect 参数只抽象“调用要求”，不抽象会改变返回 ABI 的 carrier。因而 `try` 与
`try(Error)` 仍直接写在返回组中，不能作为当前 `E: effect` 的实参；未来 async color 落地后可在
保持返回表示明确的前提下扩展 effect 值集合。

函数类型使用同一位置表达 effect，例如 `(i32)(unsafe): i32`。因此 pure 与 unsafe callable 是
不同类型，高阶函数可以用 `(E)` 约束并从实参 callable 的签名推断 E。

`.try` 和 `throw` 在控制流意义上产生参数化的 try effect；最近能够执行
`FromResidual(R)` / `FromError(E)` 的 Try 返回边界负责处理它。因此 `(try(E)): T` 函数可以在内部
使用 `.try`，但调用本身只是取得一个 `Result(T, E)`，不会向调用者泄漏 try effect。类似地，
`.await` 产生的异步挂起 effect 由 Future 函数或
`async { ... }` 处理并转换成 Future。`unsafe`、`try(R)` 与异步挂起会共享 effect 行和高阶函数
转发机制，但各自的 handler 语义不同。

首批内建 effect 限定为 `unsafe`、`try(R)` 和异步挂起。生成器加入时可再增加参数化的
...
内存安全核心之外的操作必须显式进入 `unsafe { ... }` handler 或 `(unsafe): T` 函数。表面上的
