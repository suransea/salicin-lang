...
`suspend(): ()` operation 的 `Async`；`core.domains` 声明 `type`、`region`、`effect`、`access` 与
`passing` 编译期 domain；`core.control`
声明 `do`、`try`、`throw`、`unsafe`、`loop` 的控制函数签名；`core.effect.handler`
...
语言协议，声明 `Clauses(Value, Answer)` 以及 compiler-derived `handle` 成员；每个 source
`effect` 自动满足它，实际 handler body 仍由编译器 lowering。
...
`type`、`region`、`effect`、`access` 与 `passing` 都是编译期 domain，描述编译期数据或调用约定；
...
| `+= -= *= /= %=` 等 | 对应 `*Assign` trait |
| `?.`、`??`、后缀 `!` | `std.flow.Chain`、`std.flow.Coalesce`、`std.flow.Unwrap` |
...
后缀 `!` 消费左侧值并通过 `std.flow.Unwrap` 提取成功载荷：
...
`raw_trap`，遵循语言的终止进程策略。该运算符不产生或传播 `Throws(E)`，也不提供恢复分支；
