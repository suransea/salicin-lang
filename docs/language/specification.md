...
let fallible(): i32 with(try(Error)) = throw Error()
let combined(pointer: Ptr(i32)): i32 with(try(Error), unsafe) = *pointer
...
`try` 是边界构造语法：`: T with(try)` 规范化为 `Option(T)`，
`: T with(try(E))` 规范化为 `Result(T, E)`。所以裸 `try` 表示没有错误 payload 的可缺失结果，带错误类型
...
`effect` kind 的实参是完整 row，包括 `pure`、`unsafe`、名义 marker 及其组合。它们不进入运行时，
但参与单态化；`pure` 是省略且没有其他约束时的默认值。effect 参数只能声明在函数或泛型 inherent
member 上，只能用于 `with(...)` 或转发给另一个 effect 编译期参数，不能作为运行时类型。
...
`try(Error)` 仍直接写在 `with(...)` 中并规范化 carrier，不能作为当前 `E: effect` 的实参；未来
async color 落地后可在
...
```sali
let f(x: i32): i32 with(try(Error)) = {
  let a = foo().try
  bar(a).try
}
```
...
```sali
let load(): Document with(try(Error)) = {
