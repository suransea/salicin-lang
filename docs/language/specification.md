...
`Option`、`Result` 和 `ResultWith` 是 `core` 根模块中的普通标准库定义；源码命名它们时需要
`use core.Option`、`use core.Result` 或 `use core.ResultWith`。它们不是 prelude 名称。
...
直接调用普通 `Throws(E)` 函数或局部函数值，可在成功类型可探测且错误类型唯一时推断 `Result(T, E)`；
无法唯一确定时，用 `use core.Result` 后的 `let result: Result(T, E) = try { ... }` 提供上下文，
...
部分应用的透明类型别名可以作为 HKT 实现目标。例如标准库用根模块中的 `ResultWith(Error)` 把二元
...
`Option(T)` 和 `Result(T, E)` 是 `core` 根模块中的普通 enum。语言为 `?.` 与 `??` 提供标准协议。
...
`Result(T, E)` 与 `Option(T)` 是 `core` 根模块中的普通 enum。`??`、`?.`、`match` 和普通方法操作
