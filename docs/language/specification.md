...
分支会按目标特化。首个捕获 action 切片还允许：完整 handler 调用位于块尾，前一条声明是带显式
函数类型的不可变闭包，且只共享捕获不可变 `Copy` 局部量。编译器把这些捕获提升为 handler 特化
参数，再把闭包注入词法 action 后执行 selective CPS；可变捕获、所有权捕获和非尾形态仍待后续 ABI。
