...
effect 是执行函数体时可能向调用上下文提出、且没有在函数体内处理的静态要求。它属于函数类型，
不是运行时返回值。当前实现的第一个显式 effect 是 `unsafe`：
...
`! unsafe` 允许函数体执行原始指针等不安全操作，并要求完整调用发生在另一个 `! unsafe` 函数或
...
`E: effect` 表示 effect 行，并用圆括号组合多项，例如 `! (unsafe, E)`；不会为 effect 参数引入
方括号或 `_` 推断语法。空行从上下文推断，不要求写占位符。

`.try` 和 `throw` 在控制流意义上产生参数化的 `try(R)` effect，其中 `R` 是 residual 类型；最近
能够执行 `FromResidual(R)` / `FromError(E)` 的 Try 返回边界负责处理它。因此返回
`Result(T, E)` 的普通函数可以在内部使用 `.try`，但调用该函数本身只是取得一个 `Result`，不会
向调用者泄漏 `try` effect。类似地，`.await` 产生的异步挂起 effect 由 Future 函数或
...
内存安全核心之外的操作必须显式进入 `unsafe { ... }` handler 或 `! unsafe` 函数。表面上的
