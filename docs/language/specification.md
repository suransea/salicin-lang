...
首批复合赋值协议为 `AddAssign`、`SubAssign`、`MulAssign`、`DivAssign`、`RemAssign`，分别对应
`+=`、`-=`、`*=`、`/=`、`%=`。协议方法可变借用 `self`、消费 `rhs` 并返回 `()`；语法直接绑定
`core.ops` 中经过验证的 lang item。同名 inherent 方法或其他 trait 不参与选择。按位与移位复合
赋值在对应 `*Assign` 协议加入 core 的版本再开放。
