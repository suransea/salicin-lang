...
unsafe handler. A body without suspension may retain a custom residual effect when it is
captureless or retains only by-value `Copy` captures. Polling it inside the corresponding handler
specializes the generated poll and resume source before runtime lowering. Residual effects with
borrowed or move-only captures, suspension, or `Throws` are not yet supported. One tail-position `await` is implemented: its operand is
