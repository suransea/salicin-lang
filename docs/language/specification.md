...
分支会按目标特化。捕获 action 切片还允许：完整 handler 调用位于块尾，前一条声明是带显式函数
类型的闭包。共享 `Copy` 捕获提升为 `borrow` 参数，可变 `Copy` 捕获提升为 `borrow(mut)` 参数，消费
owned root 的捕获提升为 `move` 参数；闭包注入词法 action 后执行 selective CPS，恢复与放弃路径都
沿用续体清理并保证 move 资源只析构一次。非相邻绑定、非尾形态和任意擦除 action 仍待后续 ABI。
