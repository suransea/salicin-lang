...
- 换行只分隔语句，不自动丢弃块尾值；显式 `;` 才强制丢弃表达式值。同一逻辑行包含多个语句时
  必须用 `;`。
- 固定关键字包括：`let`、`struct`、`enum`、`trait`、`extend`、`pub`、
...
- 调用可以连续提供多个尾随闭包，每个闭包应用一个新的参数组；尾随闭包也可以写成
  ...
  `while { condition } do { body }` 在执行前测试，`do { body } while { condition }`
  在执行后测试。
...
公开声明不能在签名或公开字段中泄漏可见性更低的类型。实体别名不会提升目标可见性；
`pub let Alias = module.Entity` 是显式重导出。完整模块规则见第 11 节。
...
- 无符号整数字面量以 `u128` 幅值解析，因而能表示完整的 `u128` 范围；负号是独立的一元
  运算，因此 `-170141183460469231731687303715884105728` 可精确表示 `i128` 最小值。
  超出目标整数范围的字面量在语义检查时报错，超过 `u128` 的幅值在词法阶段报错。
...
`Option` 和 `Result` 是 `core` 根模块中的普通标准库定义；源码使用短名时需要
`let Option = std.Option` 或 `let Result = std.Result`。它们不是 prelude 名称。`Result` 按错误类型优先柯里化为
...
展开必须独占这一对括号；展开后得到的 label、内部 identity、传递模式和类型参与普通参数匹配。
schema 本身不是运行时类型。首版关联 parameter schema 只由编译器派生，不提供动态索引、遍历、
嵌套或用户实现语法。

经过验证的 variadic control intrinsic 可声明 `...Cases: parameters`：`parameters` 的每个
元素仍描述一个完整运行时参数组 schema，而前导 `...` 把 `Cases` 提升为这类 schema 的 pack。
...
`std.effect.Throws(Error)` 声明普通 abort operation `raise(move error: Error): Never`。这个
...
无法唯一确定时，用 `let Result = std.Result` 后的
`let result: Result(E)(T) = try { ... }` 提供上下文，
...
identity、传播和类型检查。派生的
`State(i32).handle get { (resume) -> ... } ... action { ... }`
...
协议。编译器只对通过 core bundle 形状校验的声明赋予 lang-item 身份。
effect 是类型级名义成员，声明名以及 `with(...)` 中路径的最后一段必须大写开头，例如 `UI`、
`State(S)`、`std.effect.Throws(E)`；小写的 `with(foo)` 不会作为兼容拼法或隐式自定义 effect
保留。`E: effect` 中的 `effect` 是开放编译期 domain；`let UI = effect {}` 这样的声明向该
domain 引入名义成员。effect row 参数按参数名解析，不受该名义命名规则限制。
控制函数的函数体由编译器提供，因为它们会建立、消除或转发非局部控制边界。普通包不能声明无函数体的
顶层函数，也不能用同名声明获得特殊语义。`core.effect.handler` 声明空结构契约
`Continuation(Input, Output)` 与 `EffectCallable(Input, Output, Answer)`；前者表示一次性续体，
后者表示携带 call/drop 入口、环境与所有权标志的擦除 handler action。其内部调用入口接收 `Input` 与
...
| `T = borrow(U)` | 建立或传入共享借用 | 只读访问 |
| `T = borrow(mut)(U)` | 建立或传入排他借用 | 可变访问 |
| 未标注 | `T: Copy` 时为 `copy`，否则为 `move` | 同对应模式 |

`borrow(mut)(T)` 是 `borrow(A)(T)` 在 `A = mut` 时的直接写法；`mut` 是 access 实参，而不是重新绑定
参数。旧式 `borrow T`、`borrow value` 和 `borrow self` 前缀拼写不属于语言语法。
...
显式 `copy` 和 `move` 永远优先于默认规则；引用类型参数则按其 `borrow(...)` 类型建立或传入借用。
特别地，即使 `i32`
实现了 `Copy`，传给 `move value: i32` 也会在语言语义上使调用方的原绑定失效。优化器可以消除
机器层面的复制，但不能让已移动绑定重新可用。这使泛型 API 能明确表达“消费一次”的协议。

`borrow(mut)(T)` 参数的实参必须是可变且可寻址的 place expression，例如可变局部、可变字段或可变解引用；
...
access 实参参与单态化。`borrow(A)` 可出现在借用类型与借用表达式中，并决定 reference 参数的借用能力。实例化为
`shared` 时等价于 `borrow(T)`，实例化为 `mut` 时等价于 `borrow(mut)(T)`。省略且没有其他约束时采用
...
`self` 必须独占第一个运行时参数组，并可使用 `self: Self`、`self: borrow(Self)`、
`self: borrow(mut)(Self)`、`move self: Self`、`copy self: Self`。实例方法必须再声明至少一个显式运行时
...
泛型关联类型构造子会保留 `type`、`access`、`region`、`usize` 与封闭值参数的 kind。实现可以
绑定直接 nominal 构造子，也可以绑定部分应用的透明 type alias；实现构造子的剩余参数必须逐项
匹配声明 kind，而不只是 arity 相同。方法模板实例化前会展开该构造子，因此借用族可以保持 access
与 region，数组族可以保持编译期长度。where 子句中的构造子等式与更一般的方程求解仍属于后续
语义能力；不能唯一决定时仍通过省略编译期参数组和命名参数由上下文消歧，不会恢复 `_` 推断占位。
...
当期望类型能唯一确定枚举，或 variant 已通过实体别名引入时，可以写短名 `Some(1)`、`None`。
...
extend A: Foo {
  let f(self: borrow(Self))(x: i32): i32 = { x }
}
```
...
extend(T: type) Cell(T) {
  let new(value: T): Cell(T) = { Cell { value: value } }
...
使用运算符语法本身不要求绑定协议短名；实现协议、在 `where` 中引用协议或直接调用协议成员时，
使用普通实体别名，例如 `let Add = std.ops.Add`。同名的用户 trait 不会获得 `+` 的 lang-item 身份。
...
别名透明保留类型构造器、trait、effect、函数重载集、全局值和 module 的原身份。
语言不提供 `*` glob 导入，避免依赖升级静默引入名称冲突。`pub let` 可以构造稳定的公共 API，
但不能把依赖包中的私有或 `pub(package)` 实体重新导出。模块级值若确实需要从成员表达式初始化，
可用括号明确写成 `let snapshot = (object.member)`，与透明实体别名区分。
...
while condition { condition } do { step() }
...
`for expression { pattern -> body }` 通过标准库 `IntoIterator`/`Iterator` trait 展开，被迭代表达式只求值
一次。pattern 每次迭代重新绑定，其 move/borrow 行为由迭代器的 `Item` 类型决定。

两个协议位于 `core.iter`，不属于 prelude：`IntoIterator` 声明关联类型 `IntoIter` 和消费
`self` 的 `into_iter`；`Iterator` 声明 GAT `Item(R: region): type`，并由
`next(R)(self: borrow(mut)(R)(Self))` 返回 `Option(Item(R))`。因此借用型迭代器可以把产出
借用限制在本次 receiver borrow 的 `R`：产出尚存活时不能再次可变借用迭代器，也不能让产出逃过
迭代器或源值。实现和约束中显式命名协议需要普通实体别名，但 `for` 语法直接绑定经过工具链校验的
lang-item 身份，同名 inherent 方法或其他 trait 不能截获展开。当前实现先接受名称绑定与 `_` 这两种
显然不可失败的 pattern；结构解构将在 irrefutability 检查覆盖名义结构后开放。

### 13.3 函数退出、赋值与不可恢复失败

`return(expression)` 立即退出当前命名函数或当前闭包，类型为 `Never`。无参形式写作
`return()`，等同返回 `()`。函数体最后的表达式是隐式返回值，但不会隐式包装进任意用户类型。
...
`??` 在左侧成功时取出 `T`，否则惰性计算右侧 `T`。它通过 `std.flow.Coalesce` trait 实现，编译器把右侧
...
标准 `Result(E)(T)` 实现令 `Ok(value)!` 返回 `value`，令 `Err(error)!` 调用 `throw(error)`。
`Option` 不实现 `Raise`，因为 `None` 不携带错误值；应先显式提供错误并转换为 `Result`。
...
`Option.Some(value)!!` 与 `Result.Ok(value)!!` 返回 `value`；标准实现遇到 `None` 或 `Err` 时调用
`raw_trap`，遵循语言的终止进程策略。`!!` 不产生或传播 `Throws(E)`，也不提供恢复分支；
可恢复控制流应继续使用 `??`、`match` 或错误 effect。用户类型可以实现 `Unwrap`，但“无法提取时
必须终止”属于协议行为契约。
...
M0 先实现有界的导入子集：`extern "C" { ... }` 中的函数只能使用整数标量、原始指针以及
返回位置的 `()`，恰有一个运行时参数组，并可用 `@link_name` 指定经过 ASCII 校验的链接名。
导入调用始终要求 `Unsafe`。稳定 C 导出、`@repr(C)` 聚合与包级 ABI 承诺仍由后续 `ABI-1`
设计；上面的导出示例描述目标语义，不表示当前编译器已经冻结该 ABI。
...
prelude 是按 edition 固定的一小组隐式别名，仅容纳普遍需要的语言基础名称。运算符 trait、拥有容器
和宿主 API 属于普通模块，通常通过 `let Name = std...Name` 建立短名，或直接使用限定路径。
升级标准库不能向旧 edition 静默加入产生冲突的 prelude 名称。
