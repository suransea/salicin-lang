...
当 action 是完整调用中第一个需要求值的运行时值时，也可以直接写尾闭包 literal；编译器先把它物化
为带 handler 参数函数类型的内部局部 binding，再进入同一捕获提升路径。action 之前还有运行时参数、
条件 action 值、跨函数传递和任意擦除 action 仍待后续 ABI。
