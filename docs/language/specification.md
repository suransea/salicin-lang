...
let fallible(): i32 with(throws(Error)) = { throw Error() }
let combined(pointer: Ptr(i32)): i32 with(throws(Error), unsafe) = {
  if pointer.is_null() { throw Error.null_pointer }
  *pointer
}
...
调用另一个具有相同 `throws(E)` 的函数会自动传播；源码不写逐调用的 `.try`。`throw error` 产生
`throws(E)` 并具有 `never` 类型。ABI 可以把该签名降为 `Result(T, E)`，但 carrier 是 lowering
细节，不是调用点显式解包的普通返回类型。
...
分支会按目标特化。捕获 action 切片还允许：完整 handler 调用位于块尾、普通 `let` 初始化式或表达式
语句，前一条声明是带显式函数类型的闭包。共享 `Copy` 捕获提升为 `borrow` 参数，可变 `Copy` 捕获提升为 `borrow(mut)` 参数，消费
owned root 的捕获提升为 `move` 参数；闭包注入词法 action 后执行 selective CPS，恢复与放弃路径都
沿用续体清理并保证 move 资源只析构一次；调用返回后提升的可变借用已经释放。非相邻绑定、嵌套调用
位置和任意擦除 action 仍待后续 ABI。
...
不能通过是否改变 carrier 把它们拆成第二套泛型机制。当前引导编译器已对参数化 `throws(E)` 做
单态化；未来加入其他控制 effect 时，同样必须完整保留 row，不能静默丢掉其中的项。

这些内建能力也必须有 edition 固定的源码契约。`core.control` 声明 `Unsafe`、`Throws(E)`、
`Shared`、`Mutable`，以及 `do`、`try`、`unsafe`、`loop` 的尾闭包函数签名；编译器只对通过
core bundle 形状校验的声明赋予 lang-item 身份。控制函数的函数体由编译器提供，因为它们会建立、
...
可恢复失败使用 `throws(E)`，需要把失败保存为值时由 `try { ... }` 产生 `Result`。首版 panic 策略固定为终止进程（abort），用于数组
...
```sali
let result: Result(Document, IoError) = try {
  load(path)
}
...
正常尾值产生 `Ok(value)`，未在内部处理的错误产生 `Err(error)`。`try` 只处理 `throws`，不处理同一
闭包中的 `unsafe`、异步或用户 marker；这些要求继续留在外围 row 中。绑定、返回值或实参提供的
`Result(T, E)` 可以确定 handler 类型；没有这类上下文时，编译器从闭包内唯一逃逸的 throws 来源
反向推断。多错误类型仍必须显式转换或用上下文消歧。

`do { ... }` 不处理错误，只转发闭包的完整 effect row。因此它不能替代 `try`；它的作用仍是创建并
立即调用一个普通尾闭包边界。即使块内的 `return` 要求生成独立 closure，`throws(E)` 的 carrier、
`unsafe` 和用户 marker 也会穿过该边界继续由外层处理。

### 15.2 `Result`、`Option` 与库协议

`Result(T, E)` 与 `Option(T)` 是普通 enum。`??`、`?.`、`match` 和普通方法操作这些值，但不会让
调用自动传播，也不会隐式包装函数尾值。返回这些类型必须显式构造 `Ok`、`Err`、`Some` 或 `None`。
旧的 `Try`、`FromResidual` 与 `FromError` traits 已删除；用户类型不能接管 `throw` 或调用传播。

后缀 `.try`、`with(try)`、`with(try(E))` 和 `try do` 均已在 1.0 前移除，不提供兼容别名。
...
两者分别具有 `Future(Result(T, E))` 与 `Result(Future(T), E)` 的形状（后者只有当错误在构造 Future
...
let narrow: Result(i32, RangeError) = i32.try_from(wide)
```
