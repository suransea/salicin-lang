...
effect 参数最终必须能抽象整个 row，包括 `throws(E)` 与异步挂起。实例化后再决定控制流变换和
ABI，不能通过是否改变 carrier 把它们拆成第二套泛型机制。引导编译器尚未完成参数化控制 effect
的单态化时，应明确拒绝该实例，而不能静默丢掉 row 中的项。
...
闭包中的 `unsafe`、异步或用户 marker；这些要求继续留在外围 row 中。当前引导实现需要由绑定、
返回值或实参的 `Result(T, E)` 期望类型确定 handler 类型；后续可以从闭包内唯一的 throws 来源
反向推断，但多错误类型仍必须显式消歧。

`do { ... }` 不处理错误，只转发闭包的 effect。因此它不能替代 `try`；它的作用仍是创建并立即调用
一个普通尾闭包边界。

### 15.2 `Result`、`Option` 与库协议

`Result(T, E)` 与 `Option(T)` 是普通 enum。`??`、`?.`、`match` 和普通方法操作这些值，但不会让
调用自动传播。作为独立的构造便利，显式返回实现 `Try(Output = T)` 的容器时，函数的正常尾值和
`return value` 可以通过 `from_output` 包成该容器；这只构造一个值，不产生或处理 effect。
标准库可以继续提供名为 `Try`、`FromResidual` 或 `FromError` 的普通抽象供容器 API 使用；它们不
控制语言的 `throws` effect，也不会让用户类型接管 `throw` 或调用传播。
...
```sali
let task = { -> fetch() }                    // 闭包类型含 with(async)
