...
effect 写在函数逻辑返回类型的最后一个编译期参数组中。没有该组就是 pure；effect 的顺序不影响
语义，同一项不能重复：

```sali
let read(pointer: Ptr(i32)): i32(unsafe) = *pointer
let optional(): i32(try) = 42
let fallible(): i32(try(Error)) = throw Error()
let combined(pointer: Ptr(i32)): i32(try(Error), unsafe) = *pointer
...
`T(unsafe)` 允许函数体执行原始指针等不安全操作，并要求完整调用发生在另一个含 `unsafe` 的函数或
...
返回类型 effect 组同时确定控制边界及其 ABI：`T(try)` 规范化为 `Option(T)`，
`T(try(E))` 规范化为 `Result(T, E)`。所以裸 `try` 表示没有错误 payload 的可缺失结果，带错误类型
...
effect 组或转发给另一个 effect 编译期参数，不能作为运行时类型。`T(unsafe, E)` 表示固定要求
...
`FromResidual(R)` / `FromError(E)` 的 Try 返回边界负责处理它。因此 `T(try(E))` 函数可以在内部
...
内存安全核心之外的操作必须显式进入 `unsafe { ... }` handler 或 `T(unsafe)` 函数。表面上的
