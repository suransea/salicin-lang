...
`Option` 和 `Result` 是 `core` 根模块中的普通标准库定义；源码命名它们时需要 `use std.Option`
或 `use std.Result`。它们不是 prelude 名称。`Result` 按错误类型优先柯里化为
...
无法唯一确定时，用 `use std.Result` 后的 `let result: Result(E)(T) = try { ... }` 提供上下文，
...
通过普通模块导入，例如 `use std.ops.Add`。同名的用户 trait 不会获得 `+` 的 lang-item 身份。
...
和宿主 API 属于普通模块，通常通过 `use std.ops...`、`use std.boxed...`、`use std.vec...` 或其他
`std...` 路径显式引入。升级标准库不能向旧 edition 静默加入产生冲突的 prelude 名称。
