...
| Deterministic left-to-right evaluation and compiler output | single-evaluation match/index tests, staged call-argument codegen tests, and repeated ledger IR/symbol comparisons | Ownership/borrow failures prevent invalid reordered access; repeated high-risk fail fixtures preserve byte-identical diagnostics | `pass/match_scalar_single_evaluation.sc`, indexed handler ordering fixture; regenerated dependency lockfiles are byte-identical | **Covered**; two independent clean compiler builds produce byte-identical LLVM IR, symbol order, and diagnostics |
