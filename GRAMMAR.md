...
v0.33 已实现泛型函数上的普通 `where T: Trait` 谓词、多个谓词和尾逗号；v0.34 又允许不涉及关联
类型的 bound method 静态分派。关联类型等式、extension 上的 where 以及 generic trait
implementation selection 仍保留为后续语义。
...
v0.33 的 parser/semantic slice 接受 `type_expr : trait_ref` 中不含关联类型等式的部分；`Output = T`
仍是保留的关联类型等式语法，不是运行时命名实参。
