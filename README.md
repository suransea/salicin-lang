...
receiver。下一步推进生命周期化 Box 借用，再以相同
allocator/drop 基础推进
`Vec(T)`；泛型 callable
...
循环外部绑定，以保证下一轮仍拥有相同的可用值。`borrow self` 与 `mut borrow self` 方法都可直接
借用临时 receiver，临时值会存活到完整调用结束；需要可变借用时，编译器生成的临时绑定会自动成为
可变绑定。bound method 的部分应用只允许捕获 `Copy` receiver 和实参。名义 `Copy` 必须以具体、同包且结构合法的
