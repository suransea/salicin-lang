...
`raw_offset(pointer, index)`，按 LLVM pointee layout 计算元素地址；v0.61 已在其上实现首个仅接收
`Copy` 元素的 `Vec(T)`，包括检查式扩容、索引和 allocation 析构。下一步扩展资源元素的 move/drop
语义与 slice 借用；泛型 callable
