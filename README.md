...
下一步让 region 参与返回引用的逃逸证明和指针 ABI，再以相同
allocator/drop 基础推进
`Vec(T)`；泛型 callable
...
函数类型和闭包类型也不实现 `Copy`。
`borrow T` / `mut borrow T` 当前只可作为局部 `let` 的类型注解，借用期仍采用词法作用域。显式 region
已经参与名称与作用域校验，但尚不延长借用；函数返回、参数值类型和数据字段中的借用必须等 region
逃逸检查落地后才开放，编译器会明确拒绝这些签名。
