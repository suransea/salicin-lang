...
文件模块首个切片尚未实现 `use` / `pub use`、`self` / `super` / `root` 路径、公开字段和跨包
可见性泄漏检查；private trait 的接收者方法候选也尚未按调用模块过滤。项目级语义诊断还没有逐
item source map。当前 `pub` 与 `pub(package)` 在同一包内都可见，但为下一步依赖包解析保留不同
语义。
