...
标准库已经从 v0.5 的 `core` 引导开始，并按 `core → alloc → std` 分层推进。v0.6 先封闭了后续
库 API 所需的字段与签名边界；下一阶段继续把运算、所有权和析构相关 trait 移入 `core`，并补齐
指针与 slice 基础。`alloc` 要等待 `Drop`、raw pointer 和分配器 ABI，平台 `std` 的 IO、文件、
环境与进程则放在 C ABI 和最小运行时之后。
