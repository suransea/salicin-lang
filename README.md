...
并由词法临时绑定保证完整调用期间的地址稳定和调用后的唯一析构。下一步推进生命周期化 Box 借用，再以相同
allocator/drop 基础推进
`Vec(T)`；泛型 callable
...
循环外部绑定，以保证下一轮仍拥有相同的可用值。共享 `borrow self` 方法可直接借用临时 receiver，
临时值会存活到完整调用结束；`mut borrow self` 仍需先绑定到可变局部。bound method 的部分应用只允许
捕获 `Copy` receiver 和实参。名义 `Copy` 必须以具体、同包且结构合法的
...
选择；默认方法已静态单态化，暂不支持泛型关联类型、完全限定调用
