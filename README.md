...
标准库按 `core → alloc → std` 分层推进。当前先建立无堆、无操作系统的 `core`；公开字段与导出
API 可见性闭环完成后继续扩充核心 trait。`alloc` 要等待 `Drop`、指针和分配器 ABI，平台 `std`
的 IO、文件、环境与进程则放在 raw pointer、C ABI 和最小运行时之后。
...
不能声明为 `copy` 参数。`Add` 已由 edition core 登记为 lang item，但重载路径目前仍要求左操作数
能静态探测为具体名义类型。`Option` / `Result` 构造器目前使用 `Option(i32).Some(1)` 这样的显式类型头；
...
文件模块当前尚未实现 glob import 和公开字段；公开 API 签名泄漏较低可见性类型的检查仍待实现。
项目级语义诊断还没有逐 item source map。
