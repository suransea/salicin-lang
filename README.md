...
函数类型和闭包类型也不实现 `Copy`。
`borrow T` / `mut borrow T` 可作为局部 `let` 的类型注解。带显式 region 的借用类型还可作为自由函数
和关联函数返回类型，但必须能追溯到同 region 的借用参数；调用者按返回引用的词法作用域保留该 loan。
首版仍拒绝借用值参数、借用字段、局部或临时值逃逸，以及返回借用的方法；借用期暂未采用完整 NLL。
