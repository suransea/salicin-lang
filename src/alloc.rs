...
fn validate_program(edition: Edition, program: &Program) -> Result<(), AllocBundleError> {
    ...
    if program.items.len() != 3
        || program.item_visibilities.len() != 3
        || program.item_origins.len() != 3
    {
        diagnostics
            .push("embedded alloc must contain exactly Box, box_new, and box_ptr".to_owned());
    } else {
        if program
            .item_visibilities
            .iter()
...
mod tests {
    ...
    fn edition_2026_alloc_bundle_parses_and_validates() {
        ...
        assert_eq!(bundle.program.items.len(), 3);
        assert!(bundle
            .program
            .item_origins
            .iter()
